...
export function RecentActionsContent({
  itemId,
  recentItems,
  ...
  return (
    ...
    >
      <CarouselContent className="-ml-0">
        {recentSlides.map((slide, slideIndex) => (
          <CarouselItem key={slideIndex} className="pl-0">
            <div className="grid grid-cols-2 gap-x-4 gap-y-6 pt-5">
              {slide.map((item, itemIndex) => {
                const itemNumber = slideIndex * 4 + itemIndex + 1;
                return (
                  <div
                    key={`${item.title}-${itemNumber}`}
                    className="relative flex h-[205px] flex-col rounded-lg border border-border/40 bg-background/70 text-center"
                  >
                    ...
                    <div className="flex min-h-0 flex-1 flex-col items-center px-3 pb-3 pt-6">
                      ...
                      <p
                        className="mt-1 line-clamp-5 text-xs leading-relaxed text-muted-foreground"
      ...
      <CarouselScrollDots api={recentActionsApi} count={recentSlides.length} />
    </Carousel>
  );
}
...
export function RecentActionsSection({
  ...
  return <SectionShell title={title}>{content}</SectionShell>;
}
