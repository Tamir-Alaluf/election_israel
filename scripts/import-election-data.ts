...
const DEFAULT_IMPORT_JSON = path.join(SCRIPT_DIR, "example-election-import.json");
...
function collectProfessionalGroupNames(payload: ElectionImportPayload): Set<string> {
  const names = new Set<string>();
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if ((payload.partyLegislations ?? []).length > 0) {
      ...
      const partyLegTitles = [
        ...new Set((payload.partyLegislations ?? []).map((r) => r.legislationTitle)),
      ];
      ...
      const partyLegOptionValues = [
        ...new Set((payload.partyLegislations ?? []).map((r) => r.optionDisplayValue)),
      ];
    ...
    if (payload.partyMembers !== undefined) {
      const names = [...new Set(payload.partyMembers.map((m) => m.partyName))];
      if (names.length > 0) {
    ...
    if (payload.partyBaseTopics !== undefined) {
      const names = [...new Set(payload.partyBaseTopics.map((r) => r.partyName))];
      if (names.length > 0) {
    ...
    if (payload.partyLegislations !== undefined) {
      const names = [...new Set(payload.partyLegislations.map((r) => r.partyName))];
      if (names.length > 0) {
    ...
    if (payload.recentActions !== undefined) {
      const names = [...new Set(payload.recentActions.map((r) => r.partyName))];
      if (names.length > 0) {
    ...
    if (payload.futurePromises !== undefined) {
      const names = [...new Set(payload.futurePromises.map((r) => r.partyName))];
      if (names.length > 0) {
    ...
    if (payload.candidateProfessionals !== undefined) {
      const names = [
        ...new Set(payload.candidateProfessionals.map((r) => r.candidateName)),
      ];
      if (names.length > 0) {
    ...
    if (payload.candidateCareerActions !== undefined) {
      const names = [
        ...new Set(payload.candidateCareerActions.map((r) => r.candidateName)),
      ];
      if (names.length > 0) {
    ...
    if (payload.candidateRecentActions !== undefined) {
      const names = [
        ...new Set(payload.candidateRecentActions.map((r) => r.candidateName)),
      ];
      if (names.length > 0) {
